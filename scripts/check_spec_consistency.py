...
def check_claude_md() -> None:
    rel_path = "GEMINI.md" if (ROOT / "GEMINI.md").is_file() else ".claude/CLAUDE.md"
    if not (ROOT / rel_path).is_file():
        return
    expect_contains(rel_path, "Stage 2.5")
    expect_contains(rel_path, "Stage 4.5")
    for forbidden in (
...
def _suite_version() -> str | None:
    """Parse the canonical suite version from `plugin.json` or `.claude/CLAUDE.md`."""
    ...
    if claude_md.is_file():
        match = re.search(rf"\*\*Suite version\*\*:\s*({_VERSION})", claude_md.read_text(encoding="utf-8"))
        if match:
...
def check_readme_sections() -> None:
    ...
    for skill in ("deep-research", "academic-paper", "academic-paper-reviewer", "academic-pipeline"):
        expect_contains(rel_path, skill)
    for forbidden in (
        "6th independent reviewer",
        "Peer review gains 6th independent reviewer",
    ):
...
def check_readme_zh_sections() -> None:
    for rel_path in ("README.zh-CN.md", "README.zh-TW.md"):
        text = read(rel_path)
        expect_contains(rel_path, "version-v3.21.1-blue")
        for skill in ("deep-research", "academic-paper", "academic-paper-reviewer", "academic-pipeline"):
            expect_contains(rel_path, skill)
...
def check_setup_docs() -> None:
    expect_contains("docs/SETUP.md", "Pandoc")
    expect_contains("docs/SETUP.zh-TW.md", "Pandoc")
